...
func defaultItemsUniqueFormat(definition *ItemsUnique, value []optional.Uint32, duplicateIndexes []int) string {
	sb := strings.Builder{}
	sb.WriteString("must have only unique elements, but had duplicates of ")
	for i, d := range duplicateIndexes {
		value[d].IfSetElse(func(actual uint32) {
...
type ItemsUnique struct {
	Format func(definition *ItemsUnique, value []optional.Uint32, duplicateIndexes []int) string
}

func (m *ItemsUnique) Validate(value []optional.Uint32, violationReceiver bad.Emitter) ok_action.Enum {
	...
	for i, v := range value {
		v.IfSetElse(func(actual uint32) {
