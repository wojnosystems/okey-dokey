...
func defaultItemsUniqueFormat(definition *ItemsUnique, value []optional.Uint8, duplicateIndexes []int) string {
	sb := strings.Builder{}
	sb.WriteString("must have only unique elements, but had duplicates of ")
	for i, d := range duplicateIndexes {
		value[d].IfSetElse(func(actual uint8) {
...
type ItemsUnique struct {
	Format func(definition *ItemsUnique, value []optional.Uint8, duplicateIndexes []int) string
}

func (m *ItemsUnique) Validate(value []optional.Uint8, violationReceiver bad.Emitter) ok_action.Enum {
