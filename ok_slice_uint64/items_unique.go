...
func defaultItemsUniqueFormat(definition *ItemsUnique, value []optional.Uint64, duplicateIndexes []int) string {
	sb := strings.Builder{}
	sb.WriteString("must have only unique elements, but had duplicates of ")
	for i, d := range duplicateIndexes {
		value[d].IfSetElse(func(actual uint64) {
...
type ItemsUnique struct {
	Format func(definition *ItemsUnique, value []optional.Uint64, duplicateIndexes []int) string
}

func (m *ItemsUnique) Validate(value []optional.Uint64, violationReceiver bad.Emitter) ok_action.Enum {
	...
	for i, v := range value {
		v.IfSetElse(func(actual uint64) {
